...
3. 这道题问的是能撑到的最大重量；总承重的限制M那一维的遍历，用true/false表示能不能到达这个重量, i.e. ```f[i][w]```表示前```i```个物品能不能撑到```w```这个重量; 
4. 并不能只用```f[i]```开一维记录前i个物品能撑到的最大重量**而不是记录当前确实撑到了多重**, e.g. ```[3,9,5,2]```, ```M=10```, 这时如果只用一维每次看最大承重,前面3个数会选择```9```,最后一个```2```就不能加进来，并没有用; 而实际上前面应该选择```[3, 5]```,然后刚好能加入最后的```2```;
5. 这是因为, 没有记录当前不大于target M的时候的最大承重是不是撑满了, 也就是到10的时候实际上并没有满，而如果开一维的话是看不出来的;
